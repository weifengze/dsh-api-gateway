 ...
 */
export const DEFAULT_MUX_WHITELIST = [
...
export const isProxyEndpointAllowed = (endpoint, whitelist) => whitelist.includes(endpoint);
/** Whether a mux stream endpoint may be opened. */
export const isMuxEndpointAllowed = (endpoint, whitelist) => whitelist.includes(endpoint);
/** Canonical endpoint of two path segments: `<namespace>/<method>`. */
export const endpointOf = (namespace, method) => `${namespace}/${method}`;
...
export const dispatchUrl = (endpoint) => `${DISPATCH_BASE}/api/${endpoint}`;
