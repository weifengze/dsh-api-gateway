...
import { DEFAULT_MUX_WHITELIST, DEFAULT_PROXY_WHITELIST, dispatchUrl, endpointOf, isProxyEndpointAllowed, } from './proxy.js';
...
export default {
    ...
    apply(ctx, config) {
         ...
         */
        const proxyUnary = async (req, res, endpoint) => {
            const connection = hostService('connection');
            if (connection === undefined)
                throw new Error('host connection service is unavailable');
        ...
        const dispatch = async (req, res) => {
            ...
            if (seg.length === 3 && seg[0] === 'sessions' && seg[2] === 'sandbox-mode' && req.method === 'POST') {
                ...
                const sessions = hostService('sessions');
        ...
        const proxyUpgrade = (req, socket, head) => {
            ...
            wss.handleUpgrade(req, socket, head, (ws) => {
                const session = new MuxSession(ws, {
                    gateway: () => hostService('typertGateway'),
        ...
        ctx.effect(() => {
            ...
            return () => {
                ...
                for (const session of muxSessions) {
                    try {
                        session.close();
                    }
                    catch { /* noop */ }
        ...
        ctx.inject(['settings'], (sctx) => {
            try {
                ...
                const scope = settings.register(SETTINGS_NAMESPACE, Config, { base: config, applies: 'live' });
                settingsScope = scope;
